...
func dropTestDatabase(dbName string, port int) {
	...
	// Clean up dolt_branch_control entries to prevent stale references from
	// causing "Failed to load database" errors under concurrent load (gt-zlv7l).
	//nolint:gosec // G201: dbName is generated by uniqueTestDBName (testdb_ + random hex)
	_, _ = db.ExecContext(ctx, fmt.Sprintf("DELETE FROM dolt_branch_control WHERE `database` = '%s'", dbName))
}
